...
import { EXAMS } from '../../config/examData';
          ...
          {EXAMS.map((e) => {
            ...
            return (
              <div key={e.type} style={{
                ...
                <div style={{ fontSize: '12.5px', color: 'var(--text-secondary)' }}>객관식 {e.mcq.length} · 단답형 {e.short.length}문항</div>
                {r ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <Link to={`/exam/${e.type}`} style={{ fontSize: '13.5px', color: e.color, fontWeight: 700 }}>다시 응시 →</Link>
                  </div>
                ) : (
